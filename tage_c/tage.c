...
void update_tage_folded_history(tage_folded_history *f, uint8_t *h)
{
    f->comp = (f->comp << 1) | h[0];
...
void ctrupdate (int8_t *ctr, int taken, int nbits)
{
    if (taken)
        {
                if ((*ctr) < ((1 << (nbits - 1)) - 1))
...
void updateghist (my_predictor *p, uint8_t **h, int dir, uint8_t *tab, int *PT)
{
        int i = 0;
...
void my_predictor_update_histories( my_predictor *p, md_addr_t pc, int taken)
{
